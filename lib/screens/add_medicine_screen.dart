...
class _AddMedicineScreenState extends State<AddMedicineScreen> {
  ...
  TimeOfDay _timeFromString(String value) {
    ...
    if (hour == null || minute == null) return const TimeOfDay(hour: 8, minute: 0);
  ...
  void _setDailyFrequency(int frequency) {
    ...
    const presets = <int, List<TimeOfDay>>{
      ...
      3: [TimeOfDay(hour: 8, minute: 0), TimeOfDay(hour: 14, minute: 0), TimeOfDay(hour: 20, minute: 0)],
      4: [TimeOfDay(hour: 8, minute: 0), TimeOfDay(hour: 12, minute: 0), TimeOfDay(hour: 16, minute: 0), TimeOfDay(hour: 20, minute: 0)],
      6: [TimeOfDay(hour: 6, minute: 0), TimeOfDay(hour: 10, minute: 0), TimeOfDay(hour: 14, minute: 0), TimeOfDay(hour: 18, minute: 0), TimeOfDay(hour: 22, minute: 0), TimeOfDay(hour: 23, minute: 30)],
    };
  ...
  Future<void> _scanMedicine() async {
    ...
    try {
      ...
      if (result == null || !result.hasData) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Veri bulunamadı.')));
        return;
      }
      ...
      if (mounted && shouldApply == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Forma uygulandı.')));
      }
    } catch (e) {
  ...
  void _applyScanResult(MedicineScanResult result) {
    ...
    if (name?.isNotEmpty == true) _nameController.text = name!;
    if (dose?.isNotEmpty == true) _dosageController.text = dose!;
    if (note?.isNotEmpty == true) _noteController.text = note!;

    setState(() {
      _selectedForm = MedicineFormHelper.formFromScanValue(form);
      if (frequency != null && frequency > 0) _setDailyFrequency(frequency);
      if (meal.contains('tok')) _withFood = true;
      else if (meal.contains('aç') || meal.contains('ac')) _withFood = false;
      ...
      if (weekdays.isNotEmpty) {
        ...
        _selectedWeekdays..clear()..addAll(weekdays);
      }
    });
  }
  ...
  Future<void> _saveCurrentMedicine() async {
    ...
    if (_selectedScheduleType == MedicineScheduleType.interval && _intervalDays < 1) {
    ...
    if (_selectedScheduleType == MedicineScheduleType.specificDays && _selectedWeekdays.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Lütfen gün seçin.')));
      return;
    }
    ...
    final medicine = Medicine(
      ...
      firstDoseTime: reminderTimesStr.isEmpty ? '08:00' : reminderTimesStr.first,
      ...
      withFood: _withFood,
      profileId: widget.initialMedicine?.profileId ?? HiveService.getActiveProfile().id,
      ...
      intervalDays: _selectedScheduleType == MedicineScheduleType.interval ? _intervalDays : 1,
      reminderTimes: reminderTimesStr,
    );

    if (widget.isEditing) {
      await NotificationService.cancelMedicineNotifications(widget.initialMedicine!);
    ...
    if (mounted) Navigator.pop(context, {'name': medicine.name, 'isEditing': widget.isEditing});
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(widget.isEditing ? 'İlacı Düzenle' : 'İlaç Ekle', style: GoogleFonts.nunito(fontSize: 18, fontWeight: FontWeight.w800)),
            Text('${HiveService.getActiveProfile().name} profili', style: GoogleFonts.nunito(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.white.withValues(alpha: 0.8))),
          ],
        ),
        actions: [
          if (_isScanning)
            ...
            Padding(
              ...
              child: TextButton.icon(
                ...
                label: Text('AI Tara', style: GoogleFonts.nunito(fontSize: 11, fontWeight: FontWeight.w800)),
              ),
            ),
        ],
      ),
        ...
        child: ListView(
          ...
          children: [
            ...
            FormSection(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(hintText: 'Örn: Metformin 500 mg'),
                  textCapitalization: TextCapitalization.words,
                  validator: (v) => v?.trim().isEmpty == true ? 'Lütfen ilaç adını girin' : null,
                ),
                const SizedBox(height: 14),
                _fieldLabel('Form'),
                FormSelector(selectedForm: _selectedForm, onFormSelected: (f) => setState(() => _selectedForm = f), isDark: isDark),
                const SizedBox(height: 14),
                _fieldLabel('Doz'),
                TextFormField(controller: _dosageController, style: GoogleFonts.nunito(fontSize: 14, fontWeight: FontWeight.w700), decoration: const InputDecoration(hintText: 'Örn: 500 mg')),
              ],
            ),
            const SizedBox(height: 14),
            FormSection(
              ...
              children: [
                ...
                TimeChips(
                  ...
                  onTimeTap: (index) async {
                    final picked = await showTimePicker(context: context, initialTime: _reminderTimes[index], builder: (c, child) => MediaQuery(data: MediaQuery.of(c).copyWith(alwaysUse24HourFormat: true), child: child!));
                    if (picked != null) setState(() { _reminderTimes[index] = picked; _reminderTimes.sort(_compareTimes); });
                  },
                ),
                const SizedBox(height: 14),
                _fieldLabel('Gün İçindeki Tekrar'),
                FrequencySelector(selectedFrequency: _dailyFrequency, onFrequencySelected: _setDailyFrequency, isDark: isDark),
                const SizedBox(height: 8),
                Text(DoseCalculator.getIntervalDescription(_dailyFrequency), style: GoogleFonts.nunito(fontSize: 12, color: AppTheme.primaryColor, fontWeight: FontWeight.w700)),
                const SizedBox(height: 14),
                _fieldLabel('Plan'),
                ScheduleTypeSelector(selectedType: _selectedScheduleType, onTypeSelected: (t) => setState(() => _selectedScheduleType = t), isDark: isDark),
                const SizedBox(height: 12),
                ScheduleSettings(
                  selectedType: _selectedScheduleType,
                  selectedWeekdays: _selectedWeekdays,
                  onWeekdayToggle: (d) => setState(() => _selectedWeekdays.contains(d) ? _selectedWeekdays.remove(d) : _selectedWeekdays.add(d)),
            ...
            FormSection(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(hintText: 'Örn: 30', suffixText: 'gün'),
                  validator: (v) {
                    if (v == null || v.trim().isEmpty) return 'Lütfen gün sayısı girin';
                    final d = int.tryParse(v.trim());
                    if (d == null || d <= 0) return 'Geçerli bir gün sayısı girin';
                    if (d > 365) return 'En fazla 365 gün girilebilir';
                    return null;
                  },
                ),
                const SizedBox(height: 14),
                _fieldLabel('Yemek bilgisi'),
                _buildToggleTile(icon: Icons.restaurant_rounded, title: 'Tok Karnına', subtitle: 'Yemekten sonra alınacak', value: _withFood, onChanged: (v) => setState(() => _withFood = v), isDark: isDark),
                const SizedBox(height: 14),
                _fieldLabel('Kart rengi'),
                MedicineColorPicker(selectedColor: _selectedColor, onColorSelected: (c) => setState(() => _selectedColor = c)),
                const SizedBox(height: 14),
                _fieldLabel('Not'),
                TextFormField(controller: _noteController, style: GoogleFonts.nunito(fontSize: 13, fontWeight: FontWeight.w700), maxLines: 2, decoration: const InputDecoration(hintText: 'Örn: Doktor tavsiyesi')),
              ],
            ),
            ...
            StockTrackingSection(
              ...
              onEnabledChanged: (v) => setState(() => _stockTrackingEnabled = v),
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryColor, minimumSize: const Size(double.infinity, 52), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
              child: Text(widget.isEditing ? 'Değişiklikleri Kaydet' : 'Kaydet ve Hatırlatıcı Kur', style: GoogleFonts.nunito(fontSize: 13, fontWeight: FontWeight.w800)),
            ),
          ],
        ),
      ),
  ...
  Widget _fieldLabel(String text) {
    ...
    return Padding(
      ...
      child: Text(text, style: GoogleFonts.nunito(fontSize: 12, fontWeight: FontWeight.w800, color: isDark ? const Color(0xFFC4B7E9) : AppTheme.primaryDark)),
    );
  }
  ...
  Widget _buildToggleTile({
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: isDark ? AppTheme.darkCard : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: isDark ? AppTheme.darkBorder : AppTheme.borderColor)),
      child: Row(
        children: [
          Container(width: 34, height: 34, decoration: BoxDecoration(color: AppTheme.accentColor, borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: AppTheme.primaryColor, size: 18)),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: GoogleFonts.nunito(fontSize: 13, fontWeight: FontWeight.w800, color: isDark ? Colors.white : AppTheme.textPrimary)),
                Text(subtitle, style: GoogleFonts.nunito(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey[500])),
              ],
            ),
          ),
          Switch.adaptive(value: value, activeThumbColor: AppTheme.primaryColor, onChanged: onChanged),
        ],
      ),
    );
  }
